...
The Flask templates load shared CSS/JS via hardcoded absolute paths with a cache-busting query string:
```html
<link rel="stylesheet" href="/shared/css/shared.css?v={{ cache_bust }}">
...
The static portfolio pages (`index.html`, `about/`, `experience/`) use hardcoded version strings (e.g. `style.css?v=acec136`). Update these manually to the new commit hash when pushing CSS/JS changes that affect the portfolio.
